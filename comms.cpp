...
{  
  ...
  // Snapshot the self-adaptation live values once per packet rather than
  // recomputing per-byte in getTSLogEntry() -- keeps the 4 values internally
  // consistent within a single burst read. See adaptation.h/logger.cpp.
  adaptation_fillOutputChannels(adaptationOutputChannelsCache);
