...
void setup(void)
{
  ...
  initialiseAll();
  adaptation_init(); //Self-adaptation: load learned-data double buffer. Config (configPage15.adapt*) is already loaded by initialiseAll()'s normal page load.
  powerLatch_init(); //Ignition-off shutdown detection for the self-latch relay -- see powerLatch.h
}
...
BEGIN_LTO_ALWAYS_INLINE(void) loop(void)
{
  uint8_t originalBatteryVoltage = currentStatus.battery10;
      ...
      adaptation_update(); //Self-adaptation: gating + learning, internally rate-limited to ~10Hz
      powerLatch_update(); //Ignition-off shutdown detection, internally rate-limited to ~20Hz
