...
void idleControl(void)
{
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_PWM_OL:      //Case 2 is PWM open loop
      ...
      //Self-adaptation idle air trim: learned drift correction (0-100% native
      //units, same scale as currentStatus.idleLoad). Returns 0 until idle
      //learning is enabled in TunerStudio. currentStatus.idleLoad is an
      //unsigned byte with no lower-bound clamp below, so the trim is applied
      //via a signed intermediate and clamped on BOTH ends here to avoid
      //underflow-wrapping the byte if the learned trim is negative and
      //idleLoad is already small. Wrapped in its own block since this sits
      //inside an unbraced switch case (avoids "crosses initialization" error
      //when other case labels follow in the same switch). See adaptation.h/.cpp.
      {
        int16_t idleLoadWithTrim = (int16_t)currentStatus.idleLoad + adaptation_getIdleAirDelta();
        if (idleLoadWithTrim < 0) { idleLoadWithTrim = 0; }
        currentStatus.idleLoad = (byte)idleLoadWithTrim;
      }
