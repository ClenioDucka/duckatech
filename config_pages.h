...
struct config15 : public config_page_t {
  ...
  //Bytes 106-119 - Self-adaptation (LTFT/idle learning) config.
  //Was part of Unused15_106_255[150]; shrunk that array by exactly 14 bytes
  //below so total struct size (176 bytes) and everything after it is unchanged.
  byte adaptEnableFuelLearn    : 1;
  byte adaptEnableIdleLearn    : 1;
  byte adaptLinkFuelToLambdaEn : 1;
  byte adaptLoadSource         : 1; // 0 = MAP (kPa), 1 = TPS (%)
  byte adaptReserved           : 4;
  byte     adaptFuelMinCltC;
  uint16_t adaptFuelMinRpm;
  uint16_t adaptFuelMaxRpm;
  uint16_t adaptFuelMinLoad;
  uint16_t adaptFuelMaxLoad;
  byte     adaptIdleMinCltC;
  uint16_t adaptIdleRpmDeadband;
  byte     adaptLearnHitThreshold;
  byte     adaptShutdownDebounceSec; // seconds fuel pump must stay confirmed-off before the EEPROM write fires. Must leave margin under your power-latch relay's hardware timeout.
  uint16_t adaptMinRunTimeSec; // fuel pump must have been continuously ON for at least this long before an OFF transition is even considered a shutdown candidate (excludes normal key-on priming pulses and short aborted starts). Default 600 (10 min).

  //Bytes 123-255
  byte Unused15_123_255[133];
