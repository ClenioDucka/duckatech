...
uint16_t correctionsFuel(void)
{
  ...
  //Self-adaptation LTFT: learned bias derived from sustained egoCorrection
  //deviation, applied as its own multiplicative correction (100=neutral).
  //Returns 100 (no-op) until fuel learning is enabled in TunerStudio. See
  //adaptation.h/.cpp.
  sumCorrections = combineCorrections(sumCorrections, adaptation_getFuelTrimPercent100());
...
TESTABLE_INLINE_STATIC int8_t correctionIdleAdvance(int8_t advance)
{
  ...
  //Self-adaptation idle ignition trim: learned + live correction, only
  //meaningful while actually at idle. Returns 0 until idle learning is
  //enabled. See adaptation.h/.cpp.
  if (currentStatus.idleOn)
  {
    advance += (int8_t)(adaptation_getIdleIgnTrim_x10() / 10);
  }
